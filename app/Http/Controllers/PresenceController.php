...
public function analyzeAttendanceByTime(Request $request)
{
    // Validasi input dari request body
    $validated = $request->validate([
        'date' => 'required|date', // Tanggal yang akan dianalisis
    ]);

    // Ambil data kehadiran pada tanggal yang diminta
    $presences = Presence::whereDate('date', $validated['date'])->get();

    // Definisikan interval waktu 30 menit
    $timeIntervals = [];
    for ($i = 0; $i < 24; $i++) {
        $start = str_pad($i, 2, '0', STR_PAD_LEFT) . ':00';
        $mid = str_pad($i, 2, '0', STR_PAD_LEFT) . ':30';
    ...
    // Mengelompokkan kehadiran berdasarkan interval waktu
    $attendanceSummary = [];
    foreach ($timeIntervals as $interval) {
        $start = $interval['start'];
        $end = $interval['end'];

        $records = $presences->filter(function ($presence) use ($start, $end) {
            return $presence->time >= $start && $presence->time < $end;
        });
    ...
    return response()->json([
        ...
        'data' => [
            'date' => $validated['date'],
            'attendance_summary_by_time' => $attendanceSummary
        ]
    ]);
}
}
